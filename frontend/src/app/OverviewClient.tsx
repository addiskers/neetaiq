...
export interface OverviewInitialData {
  ...
  dossier: DossierRow[];
  partyPerformance: any[];
  turnoutByDistrict: any[];
  marginDistribution: any[];
  categoryMix: any[];
  historicalWave: any;
  closestContests: any[];
  notaImpact: any[];
...
export default function OverviewClient({ initialData }: OverviewClientProps) {
  ...
  const [partyPerformance, setPartyPerformance] = useState<any[]>(initialData?.partyPerformance ?? []);
  const [turnoutByDistrict, setTurnoutByDistrict] = useState<any[]>(initialData?.turnoutByDistrict ?? []);
  const [marginDistribution, setMarginDistribution] = useState<any[]>(initialData?.marginDistribution ?? []);
  const [categoryMix, setCategoryMix] = useState<any[]>(initialData?.categoryMix ?? []);
  const [historicalWave, setHistoricalWave] = useState<any>(initialData?.historicalWave ?? null);
  const [closestContests, setClosestContests] = useState<any[]>(initialData?.closestContests ?? []);
  const [notaImpact, setNotaImpact] = useState<any[]>(initialData?.notaImpact ?? []);
  const [crorepati, setCrorepati] = useState<any>(initialData?.crorepati ?? null);
  const [genderDemo, setGenderDemo] = useState<any[]>(initialData?.genderDemo ?? []);
  const [educationData, setEducationData] = useState<any[]>(initialData?.educationData ?? []);
  const [criminalData, setCriminalData] = useState<any>(initialData?.criminalData ?? null);
  const [marginTurnout, setMarginTurnout] = useState<any[]>(initialData?.marginTurnout ?? []);
  const [countdown, setCountdown] = useState<any>(initialData?.countdown ?? null);
          ...
          {genderDemo.length > 0 ? (
            <>
              <div className="text-center mb-2">
                <div className="text-2xl font-extrabold text-[#111827]">{formatNum(stats?.total_electors ?? genderDemo.reduce((s, g) => s + g.count, 0))}</div>
