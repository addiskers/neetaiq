...
export default function PredictionsClient({ initialData }: PredictionsClientProps) {
  const { currentElection } = useFilters();
  const [summary, setSummary] = useState<any>(initialData?.summary ?? null);
  const [predictions, setPredictions] = useState<any[]>(initialData?.predictions ?? []);
  const [battlegrounds, setBattlegrounds] = useState<any[]>(initialData?.battlegrounds ?? []);
