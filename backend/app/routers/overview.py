...
def _filter_constituencies(db: Session, eid: int, Constituency, District,
                            district: Optional[str] = None, ac_no: Optional[int] = None):
...
def list_elections(models=Depends(models_dependency), db: Session = Depends(get_db)):
    Election, *_ = models
...
def get_countdown(
    ...
    now = datetime.now()
    target = datetime.combine(election.election_date, datetime.min.time().replace(hour=7))
    diff = target - now
...
def list_states(db: Session = Depends(get_db)):
    from app.models.westbengal import WBElection, WBConstituency
    from app.models.assam import AssamElection, AssamConstituency
    from app.models.puducherry import PuducherryElection, PuducherryConstituency
    from app.models.tamilnadu import TamilNaduElection, TamilNaduConstituency
    results = []
    for ElectionCls, ConstsCls in [
        (WBElection, WBConstituency),
        (AssamElection, AssamConstituency),
        (PuducherryElection, PuducherryConstituency),
        (TamilNaduElection, TamilNaduConstituency),
    ]:
        e = db.query(ElectionCls).order_by(ElectionCls.year.desc()).first()
        if not e:
            continue
        total = db.query(func.sum(ConstsCls.total_electors)).filter(
            ConstsCls.election_id == e.id
        ).scalar() or 0
        count = db.query(ConstsCls).filter(ConstsCls.election_id == e.id).count()
        results.append(StateOverview(state=e.state, constituency_count=count, total_electors=total))
    return results
...
def get_constituency(
    ...
    c = db.query(Constituency).filter(
        Constituency.ac_no == ac_no, Constituency.election_id == eid,
    ).first()
    if not c:
        raise HTTPException(status_code=404, detail="Constituency not found")
    return ConstituencyOut(
        id=c.id, ac_no=c.ac_no, name=c.name,
        district_name=c.district.name, category=c.category,
        total_electors=c.total_electors, total_votes_polled=c.total_votes_polled,
...
def get_dossier_table(
    ...
    Election, District, Constituency, Candidate, Party = models
    eid = _resolve_eid(election_id, db, Election)
    q = db.query(Candidate).filter(Candidate.election_id == eid, Candidate.is_nota == False)
    if ac_no or district:
        cs = _filter_constituencies(db, eid, Constituency, District, district, ac_no)
    ...
    return [
        DossierRow(
            ...
            position=c.position, votes_total=c.votes_total,
            vote_pct=float(c.vote_pct) if c.vote_pct else None,
            declared_assets=c.declared_assets, liabilities=c.liabilities,
            criminal_cases=c.criminal_cases or 0, image_url=c.image_url,
        )
        for c in candidates
    ]
...
def get_stats(
    ...
    cand_q = db.query(Candidate).filter(Candidate.election_id == eid, Candidate.is_nota == False)
    if c_ids:
...
def get_turnout_by_constituency(
    election_id: Optional[int] = None,
    models=Depends(models_dependency),
    db: Session = Depends(get_db),
):
    ...
    return [
        {
            "ac_no": c.ac_no, "name": c.name, "district": c.district.name,
...
def get_turnout_by_district(
    ...
    for d in districts_db:
        ...
        result.append({
            "district": d.name, "total_electors": total_electors,
            "total_votes_polled": total_votes, "turnout": turnout,
...
def get_party_performance(
    ...
    for c in candidates:
        ...
        if pid not in party_data:
            party_data[pid] = {
                "party": c.party.name, "abbr": c.party.abbr, "color": c.party.color,
                "seats_contested": 0, "seats_won": 0, "total_votes": 0,
            }
...
def get_margin_distribution(
    ...
    Election, District, Constituency, Candidate, Party = models
    eid = _resolve_eid(election_id, db, Election)
    filtered = _filter_constituencies(db, eid, Constituency, District, district, ac_no)
...
def get_closest_contests(
    ...
    for c in cs:
        ...
        result.append({
            "name": c.name, "margin": c.winning_margin,
            "winner": winner.name if winner else None,
...
def get_nota_impact(
    ...
    cs = db.query(Constituency).filter(
        Constituency.election_id == eid, Constituency.winning_margin != None,
    ).all()
    result = []
    for c in cs:
        ...
        result.append({
            "name": c.name, "margin": c.winning_margin,
            "nota_votes": nota_votes, "nota_pct": nota_pct,
...
def get_crorepati_candidates(
    ...
    candidates = db.query(Candidate).filter(
        ...
        Candidate.declared_assets >= 10000000,
    ).all()
    party_data = {}
    for c in candidates:
        abbr = c.party.abbr if c.party else "IND"
        if abbr not in party_data:
            party_data[abbr] = {
                "party": abbr, "color": c.party.color if c.party else "#808080",
                "count": 0, "total_assets": 0,
            }
...
def get_gender_demographics(
    ...
    if male + female + third > 0:
        ...
        return [{"gender": g, "count": v, "pct": round(v / total * 100, 1)} for g, v in rows if v > 0]
    q = db.query(Candidate).filter(Candidate.election_id == eid, Candidate.is_nota == False)
    if ac_no or district:
        c_ids = [c.id for c in cs]
        q = q.filter(Candidate.constituency_id.in_(c_ids))
...
def get_margin_vs_turnout(
    ...
    for c in cs:
        ...
        if margin_pct < 2:
            buckets["< 2%"].append(turnout)
        elif margin_pct < 5:
            buckets["2-5%"].append(turnout)
        elif margin_pct < 10:
            buckets["5-10%"].append(turnout)
        elif margin_pct < 15:
            buckets["10-15%"].append(turnout)
        else:
            buckets["> 15%"].append(turnout)
    return [
        {
            "range": label,
            "avg_turnout": round(sum(t) / len(t), 1) if t else 0,
            "count": len(t),
        }
...
def get_places_to_watch(
    ...
    close = (
        db.query(Constituency)
        .filter(
            Constituency.election_id == prev.id,
            Constituency.winning_margin != None,
            Constituency.winning_margin < 10000,
        )
        .order_by(Constituency.winning_margin)
        .limit(15)
        .all()
...
def get_swing_analysis(
    ...
    Election, District, Constituency, Candidate, Party = models
    eid = _resolve_eid(election_id, db, Election)
    all_elections = db.query(Election).order_by(Election.year).all()
    ...
    all_p = sorted(
        set(list(s_prev.keys()) + list(s_curr.keys())),
        key=lambda p: s_curr.get(p, 0) + s_prev.get(p, 0), reverse=True
    )[:8]
    return [{
        "party": p,
        "year_prev": e_prev.year, "year_curr": e_curr.year,
        "seats_2016": s_prev.get(p, 0), "seats_2021": s_curr.get(p, 0),
...
def get_constituency_tracker(
    ...
    prev_year = None
    if election:
        e_prev = db.query(Election).filter(Election.year < election.year).order_by(Election.year.desc()).first()
        if e_prev:
            prev_year = e_prev.year
            for w in db.query(Candidate).filter(
                Candidate.election_id == e_prev.id, Candidate.position == 1, Candidate.is_nota == False
            ).all():
                winner_prev[w.constituency.name.upper()] = {
                    "name": w.name,
    ...
    for c in constituencies:
        ...
        cand_list = [{
            ...
            "position": ca.position, "votes_total": ca.votes_total,
            ...
            "education": ca.education, "age": ca.age,
            "gender": ca.gender, "image_url": ca.image_url,
        } for ca in candidates]

        entry = {
            "ac_no": c.ac_no, "name": c.name, "district": c.district.name,
            "category": c.category, "total_electors": c.total_electors,
            "total_votes_polled": c.total_votes_polled,
            "turnout_pct": float(c.turnout_pct) if c.turnout_pct else None,
            "winning_margin": c.winning_margin,
            "candidate_count": len(cand_list), "candidates": cand_list,
        }

        prev = winner_prev.get(c.name.upper())
        if prev:
            entry["winner_prev"] = prev
        result.append(entry)
