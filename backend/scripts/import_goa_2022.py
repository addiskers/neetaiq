"""Import Goa 2022 election data.

Data sources (all under Goa/Goa_2022/):
  - Results:  10-Detailed Results.xlsx
  - Electors: Electors & Voters Data_2022.xlsx
  - CSV bridge: Goa_candidates_2022.csv
  - JSON profiles: 2022_candidate_profile.json
  - Mapping: Goa/Goa_Data_Mapping.xlsx

Usage: cd backend && python -m scripts.import_goa_2022
"""
...
DATA_DIR     = os.path.join(PROJECT_ROOT, "Goa", "Goa_2022")
...
CANDIDATES_CSV = os.path.join(DATA_DIR, "Goa_candidates_2022.csv")

PARTY_COLORS = {
    "BJP": "#FF9933", "INC": "#00BFFF", "MGP": "#006400", "AAP": "#0066CC",
    "GFP": "#FF4500", "NCP": "#004080", "IND": "#808080", "AITC": "#00FF00",
    "BSP": "#0000FF", "CPI": "#FF4444", "CPI(M)": "#CC0000", "MAG": "#006400",
    "TMC": "#00FF00", "RGP": "#800080",
}
...
def load_electors():
    # Cols: State/UT(0), District(1), AC NO(2), AC Name(3), MALE(4), FEMALE(5), THIRD(6), TOTAL(7)
...
def run():
    ...
    # Results: STATE(0), District(1), AC NO(2), AC NAME(3), CANDIDATE NAME(4), SEX(5),
    #          AGE(6), CATEGORY(7), PARTY(8), SYMBOL(9), GENERAL(10), POSTAL(11),
    #          TOTAL(12), %(13), ELECTORS(14)
    # Candidate name has rank prefix: "1 CANDIDATE NAME"
    ...
    for row in result_rows[1:]:
        ...
        # Strip rank prefix "1 CANDIDATE NAME" → "CANDIDATE NAME"
        ...
        party_raw = str(row[8]).strip() if row[8] else None
        ac_data[ac_no]["candidates"].append({
            "name": clean_name or None,
            "sex": "M" if "MALE" in sex_raw else ("F" if "FEMALE" in sex_raw else None),
            "age": int(row[6]) if row[6] else None,
            "category": "GEN" if cat_raw == "GENERAL" else cat_raw,
            "party": party_raw,
    ...
    with Session(engine) as session:
        if session.query(Election).filter_by(state="Goa", year=2022).first():
            print("Goa 2022 already exists — skipping.")
            return

        election = Election(state="Goa", year=2022, type="Assembly",
                            name="Goa Legislative Assembly Election 2022")
