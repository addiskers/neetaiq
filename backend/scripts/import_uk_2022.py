"""Import Uttarakhand 2022 election data.

Results (10-Detailed Results.xlsx, Worksheet), header row 0, data from row 1:
  State(0), AC_NO(1)[int], AC_NAME(2), CandName(3), SEX(4)[MALE/FEMALE],
  AGE(5), CATEGORY(6), PARTY(7), SYMBOL(8), GenVotes(9), PostalVotes(10),
  TotalVotes(11), %Votes(12), TotalElectors(13)

Electors (Electors & Voters _UT_CEO_2022.xlsx, Table 1), header row 0:
  District(0), AC_No(1), AC_Name(2), Male(3), Female(4), Third(5), Total(6)

Note: 2022 results have no district column — district sourced from electors map.
NOTA: party.strip().upper()=="NOTA" or cand_name.strip().upper()=="NOTA"

Usage: cd backend && python -m scripts.import_uk_2022
"""
...
DATA_DIR       = os.path.join(PROJECT_ROOT, "Uttarakhand", "Uttarakhand_2022")
...
ELECTORS_FILE  = os.path.join(DATA_DIR, "Electors & Voters _UT_CEO_2022.xlsx")
PROFILES_JSON  = os.path.join(DATA_DIR, "2022_candidate_profile.json")
CANDIDATES_CSV = os.path.join(DATA_DIR, "Uttarakhand_candidates_2022.csv")
...
def load_ac_mapping():
    ...
    wb.close()
    name_to_ac = {}
    # row[0]=header, data from rows[1:]
    # cols: (AC_No_2012, AC_Name_2012, None, AC_No_2017, AC_Name_2017, None, AC_No_2022, AC_Name_2022, ...)
    for row in rows[1:]:
        if not row[0]:
            continue
        ac_no = int(row[0])
        for col in [1, 4, 7]:
            if col < len(row) and row[col]:
                ...
                for nm in [v, stripped]:
                    ...
                    # Register CANTONMENT full form alongside CANTT. abbreviation
...
def load_profiles(name_to_ac):
    ...
    for p in profiles:
        ...
        for kname in dict.fromkeys(n for n in [json_name, csv_name] if n):
            ...
            # Also index by name with leading parenthetical stripped
...
def _strip_name(name: str) -> str:
    """Strip leading serial number and parenthetical prefixes like (Dr), (Retd)."""
    name = re.sub(r"^\d+\s+", "", name.strip())           # "1 Dhan Singh" → "Dhan Singh"
    name = re.sub(r"^\s*\([^)]*\)\s*", "", name).strip()  # "(Dr) Dhan" → "Dhan"
    return name
...
def lookup_profile(ac_no, name, profile_map, word_map):
    ...
    if prof: return prof
    # Try progressively cleaned versions of the name
...
def run():
    ...
    result_rows = list(wb["Worksheet"].iter_rows(values_only=True))
    wb.close()
    ...
    for row in result_rows[1:]:
        if not isinstance(row[1], (int, float)):
            continue
        ac_no     = int(row[1])
        cand_name = re.sub(r"^\d+\s+", "", str(row[3]).strip() if row[3] else "")
        party_raw = str(row[7]).strip() if row[7] else ""
        is_nota   = party_raw.upper() == "NOTA" or cand_name.strip().upper() == "NOTA"
        # district from electors map (no district col in 2022 results)
        district  = em.get(ac_no, {}).get("district", "Unknown")
    ...
    with Session(engine) as session:
        if session.query(Election).filter_by(state="Uttarakhand", year=2022).first():
            print("Uttarakhand 2022 already exists — skipping.")
            return

        election = Election(state="Uttarakhand", year=2022, type="Assembly",
                            name="Uttarakhand Legislative Assembly Election 2022")
