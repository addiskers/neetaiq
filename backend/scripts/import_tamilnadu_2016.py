"""Import Tamil Nadu 2016 election data.

Data sources (all under tamil_nadu/Tamil Nadu_2016/):
  - Results: Detailed Results_2016.xlsx
  - Electors: Dist & AC Electors&Voters_Data_2016.xlsx
  - Party names: List Of Political Parties Participated_2016.xlsx
  - Candidate profiles: my_neta_2016_candidate_profile.json
  - Candidate bridge: my_neta_tamilnadu_candidates_2016.csv

Usage: cd backend && python -m scripts.import_tamilnadu_2016
"""
...
DATA_DIR = os.path.join(PROJECT_ROOT, "tamil_nadu", "Tamil Nadu_2016")

RESULTS_FILE   = os.path.join(DATA_DIR, "Detailed Results_2016.xlsx")
ELECTORS_FILE  = os.path.join(DATA_DIR, "Dist & AC Electors&Voters_Data_2016.xlsx")
PARTIES_FILE   = os.path.join(DATA_DIR, "List Of Political Parties Participated_2016.xlsx")
PROFILES_JSON  = os.path.join(DATA_DIR, "my_neta_2016_candidate_profile.json")
CANDIDATES_CSV = os.path.join(DATA_DIR, "my_neta_tamilnadu_candidates_2016.csv")
...
# 2016 party file: abbr=col[3], name=col[2] (different column order from 2011/2021)
# Aliases for party abbreviations found in results
...
def load_electors():
    ...
    for row in rows[1:]:
        if not any(row):
            continue
        # District name may appear in col[0] when AC No col is empty
        if row[0] and not row[1]:
            current_district = re.sub(r"\s+", " ", str(row[0]).strip()).upper()
            continue
        ...
        if ac_no_raw is None:
            # Maybe district row where col[0] has a name
            if row[0]:
                val = str(row[0]).strip()
                if not val.isdigit():
        ...
        # Update district from col[0] if it's a name
        if row[0]:
            val = str(row[0]).strip()
            if not val.isdigit() and len(val) > 2:
                current_district = val.upper()
            dist_by_ac[ac_no] = current_district
        else:
            dist_by_ac[ac_no] = current_district
        ...
        em[ac_no] = {
            "district": dist_by_ac[ac_no],
            "name": re.sub(r"\s+", " ", str(row[2]).strip()).upper() if row[2] else f"AC_{ac_no}",
            "male_electors": safe_int(row[3]),
            "female_electors": safe_int(row[4]),
            "third_gender_electors": safe_int(row[5]),
            "total_electors": safe_int(row[6]),
        }
    return em


def load_party_names():
    # 2016 party file cols: SL.No(0), Party Type(1), Party Name(2), Party Abbreviation(3)
    wb = openpyxl.load_workbook(PARTIES_FILE, read_only=True)
    rows = list(wb[wb.sheetnames[0]].iter_rows(values_only=True))
    wb.close()
    return {str(r[3]).strip(): str(r[2]).strip() for r in rows[1:] if r[2] and r[3]}
...
def run():
    ...
    wb.close()
    # Cols: Constituency No.(0), Constituency Name(1), Candidate Name(2), Candidate Sex(3),
    #       Candidate Age(4), Candidate Category(5), Party Name(6),
    #       VALID VOTES in General(7), VALID VOTES in Postal(8), Total Valid Votes(9),
    #       Total Electors(10), Total Votes(11)

    with Session(engine) as session:
        if session.query(Election).filter_by(state="Tamil Nadu", year=2016).first():
            print("Tamil Nadu 2016 already exists — skipping.")
            return

        election = Election(state="Tamil Nadu", year=2016, type="Assembly",
        ...
        for row in result_rows[1:]:
            ...
            if ac_no not in ac_data:
                cat = str(row[5]).strip().upper() if row[5] else "GEN"
                ac_data[ac_no] = {
                    "name": str(row[1]).strip() if row[1] else "",
                    "category": "GEN" if cat == "GENERAL" else cat,
                    "total_electors_from_results": int(row[10]) if row[10] else None,
                    "candidates": [],
                }
            sex = str(row[3]).strip().upper() if row[3] else ""
            ac_data[ac_no]["candidates"].append({
                ...
                "total_votes": int(row[9]) if row[9] else 0,
            })
