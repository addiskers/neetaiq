"""Import Gujarat 2017 election data.

Results columns (0-indexed, header at row[0], data from row[1]):
  District(0), AC_No(1), AC_Name(2), CandidateName(3) [ALL CAPS],
  Sex(4), Age(5), Category(6), Party(7) [leading space],
  GenVotes(8), PostalVotes(9), TotalVotes(10),
  TotalElectors(11), skip(12), %Votes(13)

Electors columns:
  State(0), District(1), AC_NO(2), AC_Name(3), Male(4), Female(5), ThirdGender(6), Total(7)

Usage: cd backend && python -m scripts.import_gujarat_2017
"""
...
DATA_DIR       = os.path.join(PROJECT_ROOT, "Gujarat", "Gujarat_2017")
...
CANDIDATES_CSV = os.path.join(DATA_DIR, "Gujarat_candidates_2017.csv")
...
def run():
    ...
    for row in result_rows[1:]:
        ...
        is_nota   = party_raw.upper() == "NOTA" or cand_name == "None of the Above"
    ...
    with Session(engine) as session:
        existing = session.query(Election).filter_by(state="Gujarat", year=2017).first()
        if existing:
            ...
            session.flush()
            print(f"Deleted existing Gujarat 2017: {n_c} candidates, {n_con} constituencies, {n_d} districts")

        election = Election(state="Gujarat", year=2017, type="Assembly",
                            name="Gujarat Legislative Assembly Election 2017")
