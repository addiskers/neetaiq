"""Import Gujarat 2022 election data.
...
Usage: cd backend && python -m scripts.import_gujarat_2022
"""
...
DATA_DIR       = os.path.join(PROJECT_ROOT, "Gujarat", "Gujarat_2022")
...
CANDIDATES_CSV = os.path.join(DATA_DIR, "Gujarat_candidates_2022.csv")
...
def load_ac_mapping():
    ...
    # Myneta 2022 uses district suffix to disambiguate duplicate AC names
    name_to_ac["KALOL-GANDHINAGAR"] = 38
    name_to_ac["KALOL-PANCHMAHALS"] = 127
    return name_to_ac
...
def run():
    ...
    for row in result_rows[1:]:
        ...
        # Strip leading candidate number: "1 Jadeja Pradhyumansinh..." -> "Jadeja Pradhyumansinh..."
    ...
    with Session(engine) as session:
        if session.query(Election).filter_by(state="Gujarat", year=2022).first():
            print("Gujarat 2022 already exists — skipping.")
            return

        election = Election(state="Gujarat", year=2022, type="Assembly",
                            name="Gujarat Legislative Assembly Election 2022")
