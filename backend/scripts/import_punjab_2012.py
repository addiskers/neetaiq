"""Import Punjab 2012 election data.

Results columns (0-indexed):
  State(0), District(1), AC_NO(2), AC_Name(3), Candidate_No(4), Candidate_Name(5),
  Sex(6), Age(7), Category(8), Party_Name(9),
  General_Votes(10), Postal_Votes(11), Total_Votes(12), Pct(13), Total_Electors(14)

Electors columns:
  State(0), District(1), AC_NO(2), AC_Name(3), Male(4), Female(5), Others(6), Total(7)

Usage: cd backend && python -m scripts.import_punjab_2012
"""
...
DATA_DIR       = os.path.join(PROJECT_ROOT, "Punjab", "Punjab_2012")
...
CANDIDATES_CSV = os.path.join(DATA_DIR, "Punjab_candidates_2012.csv")
...
PARTY_COLORS = {
    ...
    "JDU": "#00CC66", "SP": "#FF0000", "RJD": "#008000",
    "SAD(M)": "#1A5276", "RMPOI": "#9932CC",
}
...
def run():
    ...
    for row in result_rows[1:]:
        ...
        is_nota   = party_raw.upper() == "NOTA" or cand_name in ("None of the Above", "NOTA")
    ...
    with Session(engine) as session:
        if session.query(Election).filter_by(state="Punjab", year=2012).first():
            print("Punjab 2012 already exists — skipping.")
            return

        election = Election(state="Punjab", year=2012, type="Assembly",
                            name="Punjab Legislative Assembly Election 2012")
        ...
        for ac_no, info in ac_data.items():
            ...
            category = cat_m.group(1).upper() if cat_m else elec.get("category", "GEN")
