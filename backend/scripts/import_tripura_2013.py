"""Import Tripura 2013 election data — full detailed results + myneta enrichment.

Sources (Tripura/Tripura_2013/):
  - Detailed Results:  Detailed_Result.xlsx
  - Electors data:     Electors & Voters Data.xlsx
  - Party list:        LIST OF PARTICIPATING POLITICAL PARTIES.xlsx
  - Candidate CSV:     Tripura_candidates_2013.csv
  - Profile JSON:      2013_candidate_profile.json
"""
...
DATA_DIR         = os.path.join(PROJECT_ROOT, "Tripura", "Tripura_2013")
DETAILED_RESULTS = os.path.join(DATA_DIR, "Detailed_Result.xlsx")
...
PROFILES_JSON    = os.path.join(DATA_DIR, "2013_candidate_profile.json")
CANDIDATES_CSV   = os.path.join(DATA_DIR, "Tripura_candidates_2013.csv")
...
def load_ac_category_map():
    """2013 AC cols (0,1) from mapping file → ac_no: category."""
...
def import_data():
    ...
    # 2013 cols: state(0), district(1), ac_no(2), ac_name(3), cand_no(4), name(5),
    # sex(6), age(7), category(8), party(9),
    # gen_votes(10), postal_votes(11), total_votes(12), vote_pct(13), total_electors(14)
    # Note: no SYMBOL column (shifted one left compared to 2023)
    ...
    with Session(engine) as session:
        election = Election(
            state="Tripura", year=2013, type="Assembly",
            name="Tripura Legislative Assembly Election 2013",
        )
        ...
        for ac_no in sorted(ac_groups.keys()):
            ...
            valid = [(r, int(r[12]) if r[12] else 0) for r in cand_rows if r[5]]
            valid.sort(key=lambda x: -x[1])

            winning_margin = (valid[0][1] - valid[1][1]) if len(valid) >= 2 else None
            total_votes    = el.get("total_votes") or (int(cand_rows[0][14]) if cand_rows and cand_rows[0][14] else None)
            total_electors = el.get("total") or (int(cand_rows[0][14]) if cand_rows and cand_rows[0][14] else None)
            ...
            for pos, (row, _) in enumerate(valid, start=1):
                ...
                session.add(Candidate(
                    ...
                    votes_total=int(row[12]) if row[12] else 0,
                    vote_pct=float(row[13]) if row[13] else None,
                    is_nota=is_nota,
                ))
...
def enrich_profiles():
    ...
    with Session(engine) as session:
        el = session.query(Election).filter_by(state="Tripura", year=2013).first()
        if not el:
            print("Tripura 2013 not found — run import_data() first")
            return
