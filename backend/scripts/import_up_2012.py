"""Import Uttar Pradesh 2012 election data.

Results columns (0-indexed, header at row[0], data from row[1]):
  District(0), AC_No(1), AC_Name(2), CandNo(3), CandName(4)[ALL CAPS],
  Sex(5), Age(6), Category(7), Party(8),
  GenVotes(9), PostalVotes(10), TotalVotes(11), %Votes(12), Electors(13)

No NOTA in 2012.

Usage: cd backend && python -m scripts.import_up_2012
"""
...
DATA_DIR       = os.path.join(PROJECT_ROOT, "Uttar Pradesh", "UTTAR PRADESH_2012")
MAPPING_FILE   = os.path.join(PROJECT_ROOT, "Uttar Pradesh", "UP_ ELection_ Mapping_Data.xlsx")
RESULTS_FILE   = os.path.join(DATA_DIR, "Detailed_Results_ECI_Report_2012.xlsx")
ELECTORS_FILE  = os.path.join(DATA_DIR, "Electors & Voters Data _ 2012.xlsx")
PROFILES_JSON  = os.path.join(DATA_DIR, "2012_candidate_profile.json")
CANDIDATES_CSV = os.path.join(DATA_DIR, "UttarPradesh_candidates_2012.csv")
...
def run():
    ...
    result_rows = list(wb["Sheet2"].iter_rows(values_only=True))
    wb.close()
    ...
    with Session(engine) as session:
        if session.query(Election).filter_by(state="Uttar Pradesh", year=2012).first():
            print("Uttar Pradesh 2012 already exists — skipping.")
            return

        election = Election(state="Uttar Pradesh", year=2012, type="Assembly",
                            name="Uttar Pradesh Legislative Assembly Election 2012")
