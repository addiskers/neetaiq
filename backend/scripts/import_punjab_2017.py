"""Import Punjab 2017 election data.

Results columns (0-indexed):
  State(0), District(1), AC_No(2), AC_Name(3), Candidate_Name(4),
  Candidate_Sex(5), Candidate_Age(6), Candidate_Category(7), Party_Name(8),
  General_Votes(9), Postal_Votes(10), Total_Valid_Votes(11),
  Total_Electors(12), Total_Votes_NOTA(13), Pct(14)

Electors columns:
  State(0), District(1), AC_NO(2), AC_Name(3), Male(4), Female(5), ThirdGender(6), Total(7)

Usage: cd backend && python -m scripts.import_punjab_2017
"""
...
DATA_DIR       = os.path.join(PROJECT_ROOT, "Punjab", "Punjab_2017")
...
CANDIDATES_CSV = os.path.join(DATA_DIR, "Punjab_candidates_2017.csv")
...
def run():
    ...
    with Session(engine) as session:
        if session.query(Election).filter_by(state="Punjab", year=2017).first():
            print("Punjab 2017 already exists — skipping.")
            return

        election = Election(state="Punjab", year=2017, type="Assembly",
                            name="Punjab Legislative Assembly Election 2017")
