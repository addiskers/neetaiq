"""Import Puducherry 2021 election data.

Data sources (all under Puducherry/Puducherry_2021/):
  - Results: Detailed Results_2021.xlsx
  - Electors: District_Constituency wise Electors & Voters Data_2021.xlsx
  - Party names: List Of Political Parties Participated_2021.xlsx
  - Candidate profiles: my_neta_2021_candidate_profile.json
  - Candidate bridge: my_neta_puducherry_candidates_2021.csv

Usage: cd backend && python -m scripts.import_puducherry_2021
"""
...
DATA_DIR = os.path.join(PROJECT_ROOT, "Puducherry", "Puducherry_2021")

RESULTS_FILE   = os.path.join(DATA_DIR, "Detailed Results_2021.xlsx")
ELECTORS_FILE  = os.path.join(DATA_DIR, "District_Constituency wise Electors & Voters Data_2021.xlsx")
PARTIES_FILE   = os.path.join(DATA_DIR, "List Of Political Parties Participated_2021.xlsx")
PROFILES_JSON  = os.path.join(DATA_DIR, "my_neta_2021_candidate_profile.json")
CANDIDATES_CSV = os.path.join(DATA_DIR, "my_neta_puducherry_candidates_2021.csv")
...
def run():
    ...
    wb.close()
    # Cols: STATE(0), AC NO.(1), AC NAME(2), CANDIDATE NAME(3), SEX(4), AGE(5), CATEGORY(6),
    #       PARTY(7), SYMBOL(8), GENERAL(9), POSTAL(10), TOTAL(11), %(12), TOTAL ELECTORS(13)
    # Note: CANDIDATE NAME has rank prefix e.g. "1 A. NAMASSIVAYAM"

    with Session(engine) as session:
        if session.query(Election).filter_by(state="Puducherry", year=2021).first():
            print("Puducherry 2021 already exists — skipping.")
            return

        election = Election(state="Puducherry", year=2021, type="Assembly",
                            name="Puducherry Legislative Assembly Election 2021")
        ...
        for row in result_rows[1:]:
            ...
            if ac_no not in ac_data:
                ...
                ac_data[ac_no] = {
                    ...
                    "category": "GEN" if cat == "GENERAL" else cat,
                    "total_electors_from_results": int(row[13]) if row[13] else None,
                    "candidates": [],
                }
            # Strip rank prefix from candidate name ("1 A. NAMASSIVAYAM" -> "A. NAMASSIVAYAM")
