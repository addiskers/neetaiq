"""Import Puducherry 2016 election data.

Data sources (all under Puducherry/Puducherry_2016/):
  - Results: Detailed Results_2016.xlsx
  - Electors: District_Constituency wise Electors & Voters Data_2016.xlsx
  - Party names: List Of Political Parties Participated_2016.xlsx
  - Candidate profiles: my_neta_2016_candidate_profile.json
  - Candidate bridge: my_neta_puducherry_candidates_2016.csv

Usage: cd backend && python -m scripts.import_puducherry_2016
"""
...
DATA_DIR = os.path.join(PROJECT_ROOT, "Puducherry", "Puducherry_2016")

RESULTS_FILE   = os.path.join(DATA_DIR, "Detailed Results_2016.xlsx")
ELECTORS_FILE  = os.path.join(DATA_DIR, "District_Constituency wise Electors & Voters Data_2016.xlsx")
...
CANDIDATES_CSV = os.path.join(DATA_DIR, "my_neta_puducherry_candidates_2016.csv")
...
PARTY_COLORS = {
    ...
    "BSP": "#0000FF", "IND": "#808080", "NR_CONGRESS": "#800080",
}

ABBR_ALIASES = {"CPM": "CPI(M)", "NR CONGRESS": "AINRC", "N.R. CONGRESS": "AINRC"}
...
def load_electors():
    # Cols: District(0), AC No(1), AC Name(2), None(3), Type(4), Male(5), Female(6), Third(7), Total(8)
...
def load_party_names():
    # Cols: Party Type(0), Party Name(1), Party Abbreviation(2)
...
def run():
    ...
    #       General Votes(7), Postal Votes(8), Total Valid Votes(9), Total Electors(10), Total Votes(11)

    with Session(engine) as session:
        if session.query(Election).filter_by(state="Puducherry", year=2016).first():
            print("Puducherry 2016 already exists — skipping.")
            return

        election = Election(state="Puducherry", year=2016, type="Assembly",
                            name="Puducherry Legislative Assembly Election 2016")
