"""Import Uttar Pradesh 2022 election data.

Results columns (0-indexed, header at row[0], data from row[1]):
  State(0), District(1), AC_NO(2), AC_NAME(3), CANDIDATE_NAME(4),
  SEX(5)[MALE/FEMALE], AGE(6), CATEGORY(7), PARTY(8), SYMBOL(9),
  GenVotes(10), PostalVotes(11), TotalVotes(12), %Votes(13), TotalElectors(14)
...
Usage: cd backend && python -m scripts.import_up_2022
"""
...
from app.models.up import Election, District, Constituency, Party, Candidate

PROJECT_ROOT   = os.path.join(os.path.dirname(__file__), "..", "..")
DATA_DIR       = os.path.join(PROJECT_ROOT, "Uttar Pradesh", "UTTAR PRADESH_2022")
MAPPING_FILE   = os.path.join(PROJECT_ROOT, "Uttar Pradesh", "UP_ ELection_ Mapping_Data.xlsx")
RESULTS_FILE   = os.path.join(DATA_DIR, "10-Detailed Results_ECI_REPORTS_2022.xlsx")
...
CANDIDATES_CSV = os.path.join(DATA_DIR, "UttarPradesh_candidates_2022.csv")

PARTY_COLORS = {
    "BJP": "#FF9933", "INC": "#00BFFF", "IND": "#808080",
    "SP":  "#FF0000", "BSP": "#0000FF", "RLD": "#00CC00",
    "AAP": "#2681C3", "NCP": "#004080", "AIMIM": "#006400",
    "CPI": "#FF4444", "CPI(M)": "#CC0000", "CPM": "#CC0000",
    "SBSP": "#FF6633", "AAAP": "#8B4513", "AIFB": "#8B0000",
}
...
def load_ac_mapping():
    ...
    name_to_ac = {}
    # row[0]=year headers, row[1]=col headers, row[2:]=data
    # cols: (2012_AC_NO, 2012_Name, None, 2017_AC_NO, 2017_Name, None, 2022_AC_NO, 2022_Name, None, note)
    for row in rows[2:]:
        if not row[0]:
            continue
        ac_no = int(row[0])
...
def run():
    ...
    with Session(engine) as session:
        if session.query(Election).filter_by(state="Uttar Pradesh", year=2022).first():
            print("Uttar Pradesh 2022 already exists — skipping.")
            return

        election = Election(state="Uttar Pradesh", year=2022, type="Assembly",
                            name="Uttar Pradesh Legislative Assembly Election 2022")
