"""Import Himachal Pradesh 2022 election data.

Results columns (0-indexed, header at row[0], data from row[1]):
  State(0), District(1), AC_NO(2), AC_NAME(3), CANDIDATE_NAME(4) [with number prefix],
...
Usage: cd backend && python -m scripts.import_hp_2022
"""
...
from app.models.himachal import Election, District, Constituency, Party, Candidate

PROJECT_ROOT   = os.path.join(os.path.dirname(__file__), "..", "..")
DATA_DIR       = os.path.join(PROJECT_ROOT, "Himachal Pradesh", "Himachal_Pradesh_2022")
MAPPING_FILE   = os.path.join(PROJECT_ROOT, "Himachal Pradesh", "HP_Election _Data_Mapping.xlsx")
...
def load_ac_mapping():
    ...
    name_to_ac = {}
    # rows[0] = year headers (2012, 2017, 2022), rows[1] = col headers, rows[2:] = data
    # cols: (2012 AC_No, 2012 AC_Name, None, 2017 AC_No, 2017 AC_Name, None, 2022 AC_No, 2022 AC_Name)
    for row in rows[2:]:
        if not row[0]:
            continue
        ac_no = int(row[0])
...
def run():
    ...
    for row in result_rows[1:]:
        ...
        cand_name = re.sub(r"^\d+\s+", "", raw_name).strip()
        party_raw = str(row[8]).strip() if row[8] else ""
        is_nota   = party_raw.upper() == "NOTA" or "nota" in cand_name.lower()
    ...
    with Session(engine) as session:
        if session.query(Election).filter_by(state="Himachal Pradesh", year=2022).first():
            print("Himachal Pradesh 2022 already exists — skipping.")
            return

        election = Election(state="Himachal Pradesh", year=2022, type="Assembly",
                            name="Himachal Pradesh Legislative Assembly Election 2022")
