"""Import Himachal Pradesh 2017 election data.

Results columns (0-indexed, header at row[0], data from row[1]):
  District(0), AC_No(1), AC_Name(2), CandNo(3), CandName(4) [ALL CAPS],
  Sex(5)[M/F], Age(6), Category(7), Party(8)[leading space],
  GenVotes(9)[leading space], PostalVotes(10), TotalVotes(11),
  Electors(12), TotalVotes+NOTA(13), %Votes(14)

NOTA: party.strip()=="NOTA" or name=="None of the Above"

Usage: cd backend && python -m scripts.import_hp_2017
"""
...
DATA_DIR       = os.path.join(PROJECT_ROOT, "Himachal Pradesh", "Himachal_Pradesh_2017")
...
CANDIDATES_CSV = os.path.join(DATA_DIR, "Himachal_Pradesh_candidates_2017.csv")
...
def run():
    ...
    with Session(engine) as session:
        if session.query(Election).filter_by(state="Himachal Pradesh", year=2017).first():
            print("Himachal Pradesh 2017 already exists — skipping.")
            return

        election = Election(state="Himachal Pradesh", year=2017, type="Assembly",
                            name="Himachal Pradesh Legislative Assembly Election 2017")
