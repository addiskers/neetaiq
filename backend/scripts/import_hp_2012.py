"""Import Himachal Pradesh 2012 election data.

Results columns (0-indexed, header at row[0], data from row[1]):
  State(0), District(1), AC_NO(2), AC_Name(3), CandNo(4), CandName(5) [ALL CAPS],
  Sex(6), Age(7), Category(8), Party(9),
  GenVotes(10), PostalVotes(11), TotalVotes(12), %Votes(13), TotalElectors(14)

No NOTA in 2012.

Usage: cd backend && python -m scripts.import_hp_2012
"""
...
DATA_DIR       = os.path.join(PROJECT_ROOT, "Himachal Pradesh", "Himachal_Pradesh_2012")
...
CANDIDATES_CSV = os.path.join(DATA_DIR, "Himachal_Pradesh_candidates_2012.csv")
...
def run():
    ...
    with Session(engine) as session:
        if session.query(Election).filter_by(state="Himachal Pradesh", year=2012).first():
            print("Himachal Pradesh 2012 already exists — skipping.")
            return

        election = Election(state="Himachal Pradesh", year=2012, type="Assembly",
                            name="Himachal Pradesh Legislative Assembly Election 2012")
