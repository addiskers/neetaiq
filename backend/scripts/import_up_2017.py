"""Import Uttar Pradesh 2017 election data.

Results columns (0-indexed, header at row[0], data from row[1]):
  District(0), AC_No(1), AC_Name(2), CandName(3)[ALL CAPS],
  Sex(4)[M/F], Age(5), Category(6), Party(7)[leading space],
  GenVotes(8)[leading space], PostalVotes(9), TotalVotes(10),
  Electors(11), TotalVotes+NOTA(12), %Votes(13)

NOTA: party.strip()=="NOTA" or name=="None of the Above"

Usage: cd backend && python -m scripts.import_up_2017
"""
...
DATA_DIR       = os.path.join(PROJECT_ROOT, "Uttar Pradesh", "UTTAR PRADESH_2017")
MAPPING_FILE   = os.path.join(PROJECT_ROOT, "Uttar Pradesh", "UP_ ELection_ Mapping_Data.xlsx")
RESULTS_FILE   = os.path.join(DATA_DIR, "Detailed Results_ECI_REPORTS_2017.xlsx")
...
CANDIDATES_CSV = os.path.join(DATA_DIR, "UttarPradesh_candidates_2017.csv")
...
def run():
    ...
    with Session(engine) as session:
        if session.query(Election).filter_by(state="Uttar Pradesh", year=2017).first():
            print("Uttar Pradesh 2017 already exists — skipping.")
            return

        election = Election(state="Uttar Pradesh", year=2017, type="Assembly",
                            name="Uttar Pradesh Legislative Assembly Election 2017")
