"""Import Uttarakhand 2017 election data.

Results (Detailed_Results_2017.xlsx, DetailedResult), header row 0, data from row 1:
  District(0), AC_No(1)[int], AC_Name(2), CandName(3)[CAPS],
  Sex(4)[M/F], Age(5), Category(6), Party(7)[leading space],
  GenVotes(8), PostalVotes(9), TotalVotes(10), TotalElectors(11)

Electors (Electors_Data_CEO_2017.xlsx, Table 1), header row 0:
  District(0), AC_No(1), AC_Name(2), PollingStations(3),
  MEN_General(4), WOMEN_General(5), THIRD_General(6), TOTAL_General(7),
  MEN_Service(8), WOMEN_Service(9), TOTAL_Service(10), Total(11)

NOTA: party.strip().upper()=="NOTA" or cand_name.strip()=="None of the Above"

Usage: cd backend && python -m scripts.import_uk_2017
"""
...
from app.models.uttarakhand import Election, District, Constituency, Party, Candidate

PROJECT_ROOT   = os.path.join(os.path.dirname(__file__), "..", "..")
DATA_DIR       = os.path.join(PROJECT_ROOT, "Uttarakhand", "Uttarakhand_2017")
MAPPING_FILE   = os.path.join(PROJECT_ROOT, "Uttarakhand", "Uttarakhand_Data_Mapping.xlsx")
RESULTS_FILE   = os.path.join(DATA_DIR, "Detailed_Results_2017.xlsx")
ELECTORS_FILE  = os.path.join(DATA_DIR, "Electors_Data_CEO_2017.xlsx")
PROFILES_JSON  = os.path.join(DATA_DIR, "2017_candidate_profile.json")
CANDIDATES_CSV = os.path.join(DATA_DIR, "Uttarakhand_candidates_2017.csv")

PARTY_COLORS = {
    "BJP": "#FF9933", "INC": "#00BFFF", "IND": "#808080",
    "BSP": "#0000FF", "AAP": "#2681C3", "NCP": "#004080",
    "CPI": "#FF4444", "CPI(M)": "#CC0000", "SP": "#FF0000",
    "UKD": "#8B008B", "UKDP": "#6B0000", "AIMIM": "#006400",
}
    ...
    for row in rows[1:]:
        ...
        for col in [1, 4, 7]:
            if col < len(row) and row[col]:
                ...
                for nm in [v, stripped]:
                    name_to_ac[_normalize(nm)] = ac_no
                    full = re.sub(r"CANTT?\.?$", "CANTONMENT", nm, flags=re.I).strip()
                    if full != nm:
                        name_to_ac[_normalize(full)] = ac_no
    return name_to_ac
...
def load_electors():
    ...
    for row in rows[1:]:
        ...
        district = str(row[0]).strip().title() if row[0] else "Unknown"
        em[ac_no] = {
            "name": name, "district": district, "category": category,
            "male_electors":         int(row[4])  if row[4]  else None,  # MEN General
            "female_electors":       int(row[5])  if row[5]  else None,  # WOMEN General
            "third_gender_electors": int(row[6])  if row[6]  else None,  # THIRD General
            "total_electors":        int(row[11]) if row[11] else None,  # Total (incl. service)
        }
    return em


def _fuzzy_ac_no(normalized_name: str, name_to_ac: dict):
    """Fallback: find AC by edit-distance ≤ 2 when direct lookup fails."""
    if not normalized_name or len(normalized_name) < 3:
        return None
    best_ac, best_dist = None, 3
    for k, ac_no in name_to_ac.items():
        if abs(len(k) - len(normalized_name)) > 2:
            continue
        d = _lev(normalized_name, k)
        if d < best_dist:
            best_dist = d
            best_ac = ac_no
    return best_ac
...
def load_profiles(name_to_ac):
    ...
    for p in profiles:
        ...
        ac_no = None
        if info:
            norm_con = _normalize(info["constituency"])
            ac_no = name_to_ac.get(norm_con) or _fuzzy_ac_no(norm_con, name_to_ac)
        if not ac_no:
            ac_raw = re.sub(r"\s*\(?(SC|ST)\)?\s*$", "", (cp.get("ac_name") or "").strip(), flags=re.I).strip()
            norm_ac = _normalize(ac_raw)
            ac_no  = name_to_ac.get(norm_ac) or _fuzzy_ac_no(norm_ac, name_to_ac)
        if not ac_no:
            continue
        ...
        for kname in dict.fromkeys(n for n in [json_name, csv_name] if n):
            ...
            kname_clean = re.sub(r"^\s*\([^)]*\)\s*", "", kname).strip()
            if kname_clean != kname:
                by_acno.setdefault((ac_no, _normalize(kname_clean)), {**p, "_csv": info})
...
_RANK_ABBRS = frozenset(["CAPT", "LT", "GEN", "COL", "BRIG", "MAJ", "CDR", "FLT",
                          "WG", "ADM", "SGT", "CPL", "DR", "SH", "SHRI", "SMT", "MRS", "MR", "SRI"])


def _strip_name(name: str) -> str:
    """Strip serial numbers, honors suffix, parentheticals, and military rank prefixes."""
    name = re.sub(r"^\d+\s+", "", name.strip())
    # Strip comma-separated honors/awards suffix: ", P.V.S.M, V.S.M" etc.
    name = re.sub(r"\s*,\s*[A-Z][A-Z.]+(\s*,\s*[A-Z][A-Z.]+)*\s*$", "", name).strip()
    # Remove ALL parentheticals: "(RETD)", "(Dr)", etc.
    name = re.sub(r"\s*\([^)]*\)\s*", " ", name).strip()
    name = " ".join(name.split())
    # Strip leading known rank/title abbreviations word by word
    parts = name.split()
    while parts and re.sub(r"[.]", "", parts[0]).upper() in _RANK_ABBRS:
        parts.pop(0)
    return " ".join(parts)
...
def _word_in_set(word: str, word_set: frozenset) -> bool:
    ...
    if len(word) >= 3:
        for w in word_set:
            if len(w) > len(word) and w.startswith(word): return True
    if len(word) >= 6:
        for w in word_set:
            if len(w) >= 3 and word.startswith(w): return True
    if len(word) >= 5:
        tol = 2  # allow 2 edits for 5+ char words (handles HABIB/HABEEB, regional spelling variants)
        for w in word_set:
            if len(w) >= 5 and _lev(word, w) <= tol: return True
    return False
...
def lookup_profile(ac_no, name, profile_map, word_map):
    ...
    if prof: return prof
    stripped = _strip_name(name or "")
    if stripped != (name or ""):
        prof = profile_map.get((ac_no, _normalize(stripped)))
        if prof: return prof
    base = re.split(r"\s*[@(]", stripped)[0].strip() if stripped else ""
    if base != stripped:
        prof = profile_map.get((ac_no, _normalize(base)))
        if prof: return prof
    rwords = frozenset(w.upper() for w in _name_words(name or ""))
    if not rwords: return None
    for (k_ac, k_words), profile in word_map.items():
        if k_ac != ac_no: continue
    ...
    for (k_ac, k_words), profile in word_map.items():
        ...
        if (len(matched) >= len(shorter) - 1
            ...
            return profile
    # Final fallback: fuzzy match on full normalized name within same AC
    norm_name = _normalize(_strip_name(name or ""))
    if len(norm_name) >= 6:
        best_prof, best_dist = None, 3
        for (k_ac, k_norm), profile in profile_map.items():
            if k_ac != ac_no: continue
            if abs(len(k_norm) - len(norm_name)) > 2: continue
            d = _lev(norm_name, k_norm)
            if d < best_dist:
                best_dist = d
                best_prof = profile
        if best_prof:
...
def run():
    ...
    # Augment name_to_ac with electors file names — avoids mapping spreadsheet typos
    for ac_no, info in em.items():
        nm = (info.get("name") or "").strip()
        if nm:
            stripped = re.sub(r"\s*\(?(SC|ST)\)?\s*$", "", nm, flags=re.I).strip()
            for n in dict.fromkeys([nm, stripped]):
                if n:
                    name_to_ac.setdefault(_normalize(n), ac_no)
    ...
    result_rows = list(wb["DetailedResult"].iter_rows(values_only=True))
    wb.close()
    ...
    for row in result_rows[1:]:
        ...
        ac_no     = int(row[1])
        district  = str(row[0]).strip().title() if row[0] else "Unknown"
        cand_name = str(row[3]).strip() if row[3] else ""
        party_raw = str(row[7]).strip() if row[7] else ""  # may have leading space
        is_nota   = party_raw.upper() == "NOTA" or cand_name.strip() == "None of the Above"
        ...
        ac_data[ac_no]["candidates"].append({
            "name":         cand_name,
            "sex":          "M" if sex_raw == "M" else ("F" if sex_raw == "F" else None),
    ...
    with Session(engine) as session:
        if session.query(Election).filter_by(state="Uttarakhand", year=2017).first():
            print("Uttarakhand 2017 already exists — skipping.")
            return

        election = Election(state="Uttarakhand", year=2017, type="Assembly",
                            name="Uttarakhand Legislative Assembly Election 2017")
