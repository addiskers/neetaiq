"""Import Gujarat 2012 election data.

Results columns (0-indexed, header at row[0], data from row[1]):
  State(0), District(1), AC_NO(2), AC_Name(3), CandNo(4) [skip],
  CandidateName(5) [ALL CAPS], Sex(6), Age(7), Category(8),
  Party(9), GenVotes(10), PostalVotes(11), TotalVotes(12), %Votes(13), TotalElectors(14)

Electors columns:
  State(0), District(1), AC_NO(2), AC_Name(3), Male(4), Female(5), Others(6), Total(7)

Usage: cd backend && python -m scripts.import_gujarat_2012
"""
...
from app.models.gujarat import Election, District, Constituency, Party, Candidate

PROJECT_ROOT   = os.path.join(os.path.dirname(__file__), "..", "..")
DATA_DIR       = os.path.join(PROJECT_ROOT, "Gujarat", "Gujarat_2012")
MAPPING_FILE   = os.path.join(PROJECT_ROOT, "Gujarat", "Gujarat_Election Data Mapping.xlsx")
RESULTS_FILE   = os.path.join(DATA_DIR, "Detailed Results_2012.xlsx")
...
CANDIDATES_CSV = os.path.join(DATA_DIR, "Gujarat_candidates_2012.csv")

PARTY_COLORS = {
    "BJP": "#FF9933", "INC": "#00BFFF", "IND": "#808080",
    "AAP": "#2681C3", "BSP": "#0000FF", "NCP": "#004080",
    "BTP": "#228B22", "AIMIM": "#006400", "SP": "#FF0000",
    "CPI": "#FF4444", "CPI(M)": "#CC0000", "JD(U)": "#00CC66",
    "JD(S)": "#1A6B3C", "SUCI": "#8B0000", "SHS": "#FF6600",
    "GPP": "#8B4513",
}
...
def run():
    ...
    with Session(engine) as session:
        existing = session.query(Election).filter_by(state="Gujarat", year=2012).first()
        if existing:
            n_c = session.query(Candidate).filter_by(election_id=existing.id).delete()
            n_con = session.query(Constituency).filter_by(election_id=existing.id).delete()
            n_d = session.query(District).filter_by(election_id=existing.id).delete()
            session.delete(existing)
            session.flush()
            print(f"Deleted existing Gujarat 2012: {n_c} candidates, {n_con} constituencies, {n_d} districts")

        election = Election(state="Gujarat", year=2012, type="Assembly",
                            name="Gujarat Legislative Assembly Election 2012")
        ...
        for ac_no, info in ac_data.items():
            ...
            real_sorted = sorted(info["candidates"], key=lambda x: x["total_votes"], reverse=True)
